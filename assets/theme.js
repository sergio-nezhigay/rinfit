...
if(Shopify && typeof Shopify === 'object') {
    ...
    Shopify.handleize = function (str) {
        str = str.toLowerCase().replace(/[-+!"#$₹%&'* ,./:;<=>?@[\\\]_`{|}~]+/g, "-").replace(/ś/g, "s").replace(/ą/g, "a").replace(/ć/g, "c").replace(/ä/g, "a").replace(/ö/g, "o").replace(/ü/g, "u").replace(/ß/g, "ss").replace(/ç/g, "c").replace(/ø/g, "o").replace(/å/g, "a").replace(/æ/g, "ae").replace(/ź/g, "z").replace(/[()°€]+/g, "").replace(/^-+|-+$/g, "");

        for(var key in theme.product_options_symbols) {
            var replace = new RegExp(key, "g");

            str = str.replace(replace, theme.product_options_symbols[key]);
        }
    ...
    ProductOptions.prototype = $.extend({}, ProductOptions.prototype, {
       ...
       //     $body.on('click', this.selectors.options + ' [data-js-option-value]', onProcess);
            ...
            $body.on('change', '[data-js-product-variants="control"]', function () {
                var $this = $(this),
                    $product = $this.parents('[data-js-product]'),
                    id = $this.find('option[data-value]:selected').attr('value'),
                    json = $product.attr('data-json-product'),
                    update_variant = null,
                    is_avilable = true;
